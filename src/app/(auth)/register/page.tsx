    ...
    <div className="relative min-h-[85vh] flex items-center justify-center px-4 py-10 overflow-hidden">
      ...
      <div className="w-full max-w-md space-y-5">
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-white/95 dark:bg-[#120e24]/95 border border-slate-200 dark:border-amber-500/30 shadow-2xl shadow-purple-950/40 space-y-5 backdrop-blur-2xl">
          ...
          {/* Social Sign-Up Icons / Buttons (Quick Sign-Up) */}
          <div className="space-y-2 pt-1">
            <div className="text-center">
              <span className="text-[11px] font-bold text-slate-500 dark:text-zinc-400">
                التسجيل السريع من خلال:
              </span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setSocialEmail(formData.email || '');
                  setSocialFullName(formData.fullName || '');
                  setSocialError('');
                }}
                ...
                title="إنشاء حساب من خلال Google"
              >
      ...
      {/* Social Registration / Connection Modal */}
      {socialModalProvider && (
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-sm rounded-3xl bg-white dark:bg-[#120e24] border border-slate-200 dark:border-amber-500/40 p-6 sm:p-7 shadow-2xl space-y-4 relative">
            <button
              type="button"
              onClick={() => setSocialModalProvider(null)}
              className="absolute left-4 top-4 p-1.5 rounded-full text-slate-400 hover:text-slate-600 dark:hover:text-white bg-slate-100 dark:bg-[#181330] cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>

            <div className="text-center space-y-1.5 pt-2">
              <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-[#181330] border border-slate-200 dark:border-purple-900/60 flex items-center justify-center mx-auto shadow-sm">
                ...
                {socialModalProvider === 'github' && (
                  <svg className="w-6 h-6 fill-current text-slate-800 dark:text-white" viewBox="0 0 24 24">
                    <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.53 1.032 1.53 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
                  </svg>
                )}
              ...
              <h2 className="text-base font-black text-slate-900 dark:text-white">
                {socialModalProvider === 'google' ? 'إنشاء حساب عبر Google' : socialModalProvider === 'github' ? 'إنشاء حساب عبر GitHub' : 'إنشاء حساب عبر Facebook'}
              </h2>
              <p className="text-xs text-slate-500 dark:text-zinc-400">
                أدخل بريدك الإلكتروني لربطه بحسابك وتفعيل الدخول المباشر
              </p>
            </div>
            ...
            <form onSubmit={handleSocialSubmit} className="space-y-3.5">
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-zinc-200 mb-1">البريد الإلكتروني</label>
                <div className="relative group">
                  <input
                    type="email"
                    required
                    ...
                    className="w-full h-11 pr-11 pl-4 rounded-xl bg-slate-50 dark:bg-[#181330] border border-slate-200 dark:border-purple-900/60 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-zinc-500 text-xs sm:text-sm focus:outline-none focus:border-amber-400"
                  />
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-zinc-200 mb-1">الاسم (اختياري)</label>
                <div className="relative group">
                  <input
                    type="text"
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full h-12 rounded-xl bg-gradient-to-r from-amber-500 via-amber-400 to-yellow-400 text-zinc-950 font-black text-xs sm:text-sm flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-amber-500/20 disabled:opacity-50"
                >
                  <span>{socialLoading ? 'جاري إنشاء الحساب والربط...' : 'متابعة وإنشاء الحساب فوراً'}</span>
